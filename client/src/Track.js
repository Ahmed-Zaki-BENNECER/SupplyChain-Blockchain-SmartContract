...
function Track() {
  ...
  const loadBlockchaindata = async () => {
    ...
    if (networkData) {
      ...
      var i;
      const medCtr = await supplychain.methods.goodCtr().call();
      const med = {};
      const medStage = [];
      for (i = 0; i < medCtr; i++) {
        med[i + 1] = await supplychain.methods.GoodStock(i + 1).call();
  ...
  const handlerSubmit = async (event) => {
    ...
    var ctr = await SupplyChain.methods.goodCtr().call();
    if (!(ID > 0 && ID <= ctr)) alert("Invalid Good ID!!!");
    else {
  ...
  return (
    <div>
      ...
      <h5>Enter Good ID to Track it</h5>
